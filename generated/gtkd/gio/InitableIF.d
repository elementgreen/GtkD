...
public interface InitableIF{
	...
	public static ObjectG newValist(GType objectType, string firstPropertyName, void* varArgs, Cancellable cancellable)
	{
		...
		auto __p = g_initable_new_valist(objectType, Str.toStringz(firstPropertyName), varArgs, (cancellable is null) ? null : cancellable.getCancellableStruct(), &err);
	 ...
	 * Throws: GException on failure.
	 */
	public static ObjectG newv(GType objectType, GParameter[] parameters, Cancellable cancellable)
	{
		GError* err = null;

		auto __p = g_initable_newv(objectType, cast(uint)parameters.length, parameters.ptr, (cancellable is null) ? null : cancellable.getCancellableStruct(), &err);
