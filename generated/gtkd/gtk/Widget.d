...
public class Widget : ObjectG, AccessibleIF, BuildableIF, ConstraintTargetIF
{
	...
	{ // gtk_widget_add_controller() takes an object reference, add a reference for EventController D object
...
{
	graphene_rect_t* outoutBounds = cast(graphene_rect_t*)Memory.malloc0(graphene_rect_t.sizeof);
...
{
	graphene_point_t* outoutPoint = cast(graphene_point_t*)Memory.malloc0(graphene_point_t.sizeof);
...
{
	graphene_matrix_t* outoutTransform = cast(graphene_matrix_t*)Memory.malloc0(graphene_matrix_t.sizeof);
 ...
 * Since: 4.8
 */
public void disposeTemplate(GType widgetType)
{
	gtk_widget_dispose_template(gtkWidget, widgetType);
}
...
{
	GdkRGBA* outcolor = cast(GdkRGBA*)Memory.malloc0(GdkRGBA.sizeof);

	gtk_widget_get_color(gtkWidget, outcolor);
...
{
	GtkRequisition* outminimumSize = cast(GtkRequisition*)Memory.malloc0(GtkRequisition.sizeof);
	GtkRequisition* outnaturalSize = cast(GtkRequisition*)Memory.malloc0(GtkRequisition.sizeof);
...
{
	return gtk_distribute_natural_allocation(extraSpace, cast(uint)sizes.length, sizes.ptr);
}
}
